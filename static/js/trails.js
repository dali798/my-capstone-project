            ...
            if (state === 'all') {
                for (const arr of Object.values(jsonData)){  
                    for (const idx in arr) {
                        ...
                        'beforeend', `<li class="list-group-item"><a href="/parks/${arr[idx]}">${arr[idx]}</a></li>`);
                    }            
                }
            }else{
                for (const i in jsonData[state]) {
                    document.querySelector('#park-container-2').insertAdjacentHTML(
                    'beforeend', `<li class="list-group-item"><a href="/parks/${jsonData[state][i]}">${jsonData[state][i]}</a></li>`);
                }
            } 
